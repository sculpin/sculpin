...
class SourcePermalinkFactory implements SourcePermalinkFactoryInterface
{
    ...
    protected function generatePermalinkPathname(SourceInterface $source)
    {
        ...
        $pathname = preg_replace(
            '/(html\.)?twig$|twig\.html$/', 'html', $pathname
        );
        ...
        switch ($permalink) {
            ...
            case 'pretty':
                if ($response = $this->isDatePath($pathname)) {
                    return implode(
                        '/', array_merge($response, array('index.html'))
                    );
                } else {
                    $pretty = preg_replace(
                        '/(\.[^\.\/]+|\.[^\.\/]+\.[^\.\/]+)$/', '', $pathname
            ...
            case 'date':
                ...
                break;
            default:
                list($year, $yr, $month, $mo, $day, $dy) = explode(
                    '-', date('Y-y-m-n-d-j', (int)$date)
                );
                ...
                $permalink = preg_replace(
                    '/:title/', $this->normalize($title), $permalink
                );
                $permalink = preg_replace(
                    '/:slug_title/', $this->normalize($slug ?: $title),
                    $permalink
                );
                $filename = $pathname;
                ...
                $permalink = preg_replace(
                    '/:slug_filename/', $this->normalize($slug ?: $filename),
                    $permalink
                );
                if (strrpos($filename, DIRECTORY_SEPARATOR) !== false) {
                    $basename = substr(
                        $filename, strrpos($filename, DIRECTORY_SEPARATOR) + 1
                    );
                } else {
                    $basename = $filename;
                }
                $prettyBasename = false !== strrpos($basename, '.') ? substr(
                    $basename, 0, strrpos($basename, '.')
                ) : $basename;
                $permalink = preg_replace(
                    '/:basename_real/', $basename, $permalink
                );
                $permalink = preg_replace(
                    '/:basename/', $prettyBasename, $permalink
                );

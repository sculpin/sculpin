...
class RegisterKernelListenersPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach (
            $container->findTaggedServiceIds('kernel.event_listener') as $id =>
            $events
        ) {
        ...
        foreach (
            $container->findTaggedServiceIds('kernel.event_subscriber') as $id
        => $attributes
        ) {
            ...
            if (!$refClass->implementsInterface($interface)) {
                throw new \InvalidArgumentException(
                    sprintf(
                        'Service "%s" must implement interface "%s".', $id,
                        $interface
                    )
                );
            }
            ...
            $definition->addMethodCall(
                'addSubscriberService', array($id, $class)
            );
        }
    }
}
